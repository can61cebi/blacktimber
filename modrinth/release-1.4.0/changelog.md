...
**New: sapling replanting.** A fourth per player toggle, off by default, replants a matching sapling where the tree stood. The species is read from the felled trunk: oak, birch, spruce, jungle, acacia and cherry plant one sapling; dark oak and pale oak plant their full 2x2 only when all four cells are clear; mangrove plants a propagule. It plants only on valid ground (dirt, grass, podzol, moss and the like, plus farmland; clay and mud for mangrove) and never over water, lava or an occupied block, so an unusual spot is simply left untouched. Toggle it in the player menu, with `/blacktimber replant`, or set the server default `default-replant` in the admin panel.
